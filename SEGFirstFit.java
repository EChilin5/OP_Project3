public class SEGFirstFit {
	...
	public SEGFirstFit(Process[] processes, int memorySize, int fitAlgorithm) {
		...
		while(!(checkLifeTimes())) {
			for(int i = 0; i < processes.length; i++) {
				...
				if(processes[i].getLifetime() == 0 && processes[i].isActive()) {
					exits(processes[i].getProcessID()); //remove process from memory
					if(timeNotMarked) {
			...
			do {
				if(processQueue.isEmpty()) {
					...
					int[] validIndexes = firstFitCheck(currentProcess.getProcessSize());
	...
	public String getMemoryMap() {
		...
		SEGFirstFit.processQueue.Node currentNode = processQueue.firstNode;
	...
	private int[] firstFitCheck(int processSize) {
		int spacesToFill = processSize;
		int[] validIndexes = invalidate(new int[spacesToFill]);
		//First fit - find first hole in memory
		int i = 0;
		int j = 0;
